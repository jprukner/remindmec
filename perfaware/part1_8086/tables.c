...
static struct displacement memory_displacements[][8] = {

    // TODO normalize this table - zero values are present when
    // first, second or both registers are not needed.
    {
        {.template = "[bx + si]", .first_register = BX, .second_register = SI},
        {.template = "[bx + di]", .first_register = BX, .second_register = DI},
        {.template = "[bp + si]", .first_register = BP, .second_register = SI},
        {.template = "[bp + di]", .first_register = BP, .second_register = DI},
        {.template = "[si]",
         .first_register = SI,
         .second_register = ZERO_REGISTER},
        {.template = "[di]",
         .first_register = DI,
         .second_register = ZERO_REGISTER},
        {.template = "[%u]",
         .first_register = ZERO_REGISTER,
         .second_register = ZERO_REGISTER},
        {.template = "[bx]",
         .first_register = BX,
         .second_register = ZERO_REGISTER},
